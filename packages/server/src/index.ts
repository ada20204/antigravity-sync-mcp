...
const TOOLS: Tool[] = [
    ...
    },
    {
        ...
        description:
            "Launch Antigravity with CDP debug ports enabled. Use this when Antigravity is not running and you need to start it before sending tasks. " +
        ...
        inputSchema: {
            ...
            properties: {
                targetDir: {
                    type: "string",
                    description:
                        "Workspace directory to open in Antigravity. " +
...
async function handleAskAntigravity(
    params: { prompt: string; mode?: string; model?: string; targetDir?: string },
    fallbackTargetDir?: string,
    progressToken?: string | number
): Promise<string> {
    const prompt = params.prompt;
    const paramTargetDir =
        typeof params.targetDir === "string" && params.targetDir.trim()
                ...
                if (!found.ok || !found.discovered) {
                    ...
                    if (shouldTryLaunch) {
                        ...
                        if (!launch.started && lastDiscoverError?.state?.startsWith("app_up")) {
                            throw new Error(
                                "Antigravity is running but CDP is not enabled. " +
                        ...
                        if (launch.started && launch.cdpVerified === false) {
                            ...
                            throw new Error(
                                `Antigravity was launched but CDP did not respond on port ${launch.port}. ` +
