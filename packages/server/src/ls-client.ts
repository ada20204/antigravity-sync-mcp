...
export async function callLsJson(
...
function collectByKeys(value: unknown, keys: Set<string>, out: string[]): void {
...
export async function resolveActiveCascadeId(discovered: DiscoveredCDP): Promise<string | undefined> {
...
function containsTerminalSignal(value: unknown): boolean {
    if (!value || typeof value !== "object") return false;
    if (Array.isArray(value)) return value.some((item) => containsTerminalSignal(item));
    ...
    for (const [key, raw] of Object.entries(obj)) {
        ...
        if (containsTerminalSignal(raw)) return true;
    }
    return false;
}

export function isTrajectoryTerminal(payload: unknown): boolean {
...
async function openReactiveStreamAtHost(params: {
    ...
    csrfToken: string;
    cascadeId: string;
}): Promise<ReactiveStreamHandle> {
    const { host, port, csrfToken, cascadeId } = params;
    const state: ReactiveStreamState = {
        connected: false,
        ended: false,
        terminal: false,
        lastEventAt: 0,
...
export async function openReactiveStream(
    discovered: DiscoveredCDP,
    cascadeId: string
): Promise<ReactiveStreamHandle | null> {
    const ls = resolveLsEndpoint(discovered);
    if (!ls?.port || !ls.csrfToken) return null;
