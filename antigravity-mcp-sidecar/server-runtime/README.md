...
This directory is a release artifact snapshot copied from `../antigravity-mcp-server/dist`.

Update process:
1. Build server: `npm --prefix antigravity-mcp-server run build`
2. Sync runtime:
   - `rm -rf antigravity-mcp-sidecar/server-runtime/dist`
   - `cp -r antigravity-mcp-server/dist antigravity-mcp-sidecar/server-runtime/dist`
