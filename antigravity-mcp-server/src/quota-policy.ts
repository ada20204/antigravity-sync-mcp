...
const MODE_CHAINS: Record<AskMode, string[]> = {
    fast: [
        "gemini-3-flash",
        "gemini-3-pro-low",
        "gemini-3-pro-high",
...
function normalizeMode(input?: string): AskMode {
    const value = (input || "").trim().toLowerCase();
    if (value === "plan" || value === "planning" || value === "deep") return "plan";
    return "fast";
}

export function buildCandidateChain(mode: AskMode, requestedModel?: string): string[] {
    const chain = [...MODE_CHAINS[mode]];
    const normalizedRequested = normalizeModelName(requestedModel);
    if (!normalizedRequested) return chain;
...
function isQuotaStale(snapshot: RegistryQuotaSnapshot | undefined, nowMs: number, staleAfterMs: number): boolean {
    if (!snapshot?.timestamp || !Number.isFinite(snapshot.timestamp)) return true;
    return nowMs - snapshot.timestamp > staleAfterMs;
}

function getRemainingFraction(snapshot: RegistryQuotaSnapshot | undefined, model: string): number | undefined {
    ...
    for (const item of models) {
        ...
        if (!match) continue;
        if (typeof item.remainingFraction === "number") return item.remainingFraction;
    }
    return undefined;
}
...
export function selectModelWithQuotaPolicy(input: ModelSelectionInput): ModelSelectionResult {
