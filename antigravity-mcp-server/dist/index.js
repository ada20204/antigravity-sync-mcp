...
import { discoverCDP, connectCDP } from "./cdp.js";
import { injectMessage, pollCompletionStatus, extractLatestResponse, stopGeneration, } from "./scripts.js";
...
// --- Tool Handlers ---
async function handleAskAntigravity(prompt, progressToken) {
    // 1. Discover CDP
    ...
    if (!discovered) {
        throw new Error("CDP not found. Ensure Antigravity is running with debug ports enabled. " +
    ...
    try {
        ...
        if (!injectResult.ok) {
            throw new Error(`Failed to inject message: ${injectResult.reason || injectResult.error}`);
        }
        ...
        while (true) {
            ...
            if (elapsed % KEEPALIVE_INTERVAL < POLL_INTERVAL) {
                ...
                const detail = totalAccepted > 0 ? ` (auto-accepted ${totalAccepted} actions)` : "";
                await sendProgressNotification(progressToken, progressCount, `${msg}${detail}`);
                msgIdx++;
            }
        ...
        const summary = totalAccepted > 0 || totalBlocked > 0
